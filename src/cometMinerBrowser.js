...
import { nuck, scot } from '@urbit/aura';
...
function sha512(bytes) {
  return Uint8Array.from(nobleSha512(bytes));
}
...
function bigIntToBytesLE(value, length = null) {
  ...
  if (value === 0n) {
    return length === null ? new Uint8Array([]) : new Uint8Array(length);
  }
...
function matAtom(atom) {
  ...
  return { bits: encoded.head.number, value: encoded.tail.number };
}
...
function buildSuiteBRing(seedBytes) {
  ...
  return { atom: bytesToBigIntLE(ringBytes) };
}

function buildSuiteCRing(ringBytes, tweakAtom) {
  const tweakEncoding = matAtom(tweakAtom);
  return {
    atom: 0x43n + (bytesToBigIntLE(ringBytes) << 8n) + (tweakEncoding.value << 520n),
  };
}
...
function deriveSuiteB(seedBytes, { life = 1, rift = 0, materialize = true } = {}) {
  ...
  if (!materialize) return { suite: 'b', cometAtom, star, prefix };
...
function deriveSuiteC(seedBytes, { tweak = 0n, life = 1, rift = 0, materialize = true } = {}) {
  ...
  const star = Number(bytesToBigIntLE(cometBytes.slice(0, 2)));
  const prefix = Number(bytesToBigIntLE(cometBytes.slice(14, 16)));
  if (!materialize) return { suite: 'c', cometAtom, star, prefix };
...
function matchesFilters(result, stars, prefix) {
  if (stars && stars.size > 0 && !stars.has(result.star)) return false;
  if (prefix !== null && result.prefix !== prefix) return false;
  return true;
}
...
export function mineComet(options = {}) {
  const {
    ...
    prefix = null,
    startSeed = randomSeed64(),
    onProgress = null,
    progressEvery = 10000n,
  } = options;
  ...
  for (;;) {
    ...
    if (onProgress && tries % progressEvery === 0n) {
      onProgress({ tries: tries.toString() });
    }
